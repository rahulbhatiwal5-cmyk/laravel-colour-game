...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        :root { --red: #e8192c; --green: #138808; --blue: #2563eb; --dark: #1a1a2e; --gray: #6b7280; --light: #f3f4f6; --shadow: 0 4px 16px rgba(0,0,0,0.10); }
        ...
        .header { background: linear-gradient(135deg, var(--blue), #1d4ed8); padding: 14px 16px 18px; border-radius: 0 0 22px 22px; }
        ...
        .withdraw-list { margin: 0 12px; }
        .withdraw-card { background: #fff; border-radius: 14px; padding: 14px; margin-bottom: 10px; box-shadow: var(--shadow); }
        .card-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; }
        .user-info { display: flex; align-items: center; gap: 10px; }
        .user-avatar { width: 40px; height: 40px; border-radius: 12px; background: linear-gradient(135deg, var(--blue), #1d4ed8); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 16px; font-weight: 800; font-family: 'Baloo 2', cursive; }
        .user-name  { font-size: 13px; font-weight: 800; color: #1f2937; }
        .user-phone { font-size: 11px; color: var(--gray); font-weight: 600; }
        .withdraw-amount { font-size: 20px; font-weight: 900; color: var(--red); font-family: 'Baloo 2', cursive; }

        .card-info { background: #f9fafb; border-radius: 10px; padding: 10px 12px; margin-bottom: 10px; }
...
<div class="app">
    ...
    <div class="header">
        <div class="header-top">
            ...
            <div class="header-title">💸 Pending Withdrawals</div>
        </div>
    </div>

    <div class="section-title">{{ $withdrawals->total() }} Requests Pending</div>
    ...
    <div class="withdraw-list">
        ...
        <div class="withdraw-card">
            <div class="card-top">
                <div class="user-info">
                    <div class="user-avatar">{{ strtoupper(substr($withdrawal->user->name, 0, 1)) }}</div>
                    <div>
                        <div class="user-name">{{ $withdrawal->user->name }}</div>
                        <div class="user-phone">📱 {{ $withdrawal->user->phone }}</div>
                    </div>
                </div>
                <div class="withdraw-amount">₹{{ number_format($withdrawal->amount, 2) }}</div>
            </div>

            <div class="card-info">
                <div class="info-row">
                    <span class="info-label">Requested At</span>
                    <span class="info-value">{{ $withdrawal->created_at->format('d M, h:i A') }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Current Balance</span>
                    <span class="info-value">₹{{ number_format($withdrawal->user->wallet->balance ?? 0, 2) }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Description</span>
                    <span class="info-value">{{ $withdrawal->description ?? 'Withdrawal Request' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">UPI ID</span>
                    <span class="info-value">{{ $withdrawal->upi_id ?? 'NA' }}</span>
                </div>
            </div>
            ...
            <div class="action-btns">
                ...
                <form method="POST" action="{{ route('admin.withdrawals.reject', $withdrawal) }}" style="flex:1;">
                    @csrf
                    <button type="submit" class="reject-btn" style="width:100%;" onclick="return confirm('Reject karoge toh balance wapas hoga!')">❌ Reject</button>
                </form>
            </div>
        </div>
        @empty
        <div class="empty-msg">🎉 Koi pending withdrawal nahi hai!</div>
        @endforelse
    </div>

    @if($withdrawals->hasPages())
    <div class="pagination">
        @if($withdrawals->onFirstPage()) <span>← Prev</span>
        @else <a href="{{ $withdrawals->previousPageUrl() }}">← Prev</a> @endif
        @foreach($withdrawals->getUrlRange(1, $withdrawals->lastPage()) as $page => $url)
            @if($page == $withdrawals->currentPage()) <span class="active-page">{{ $page }}</span>
            @else <a href="{{ $url }}">{{ $page }}</a> @endif
        @endforeach
        @if($withdrawals->hasMorePages()) <a href="{{ $withdrawals->nextPageUrl() }}">Next →</a>
...
<div class="bottom-nav">
    ...
    <a href="{{ route('admin.withdrawals') }}" class="nav-item active"><div class="nav-icon">💸</div><div class="nav-label">Withdraw</div></a>
